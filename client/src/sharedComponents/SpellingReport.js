...
export default class CompReport extends React.Component {
  ...
  isStatusGradable(wordIdx) {
    ...
    return (
      this.state.spellingObj.words[wordIdx] !==
        this.state.spellingObj.responses[wordIdx] && this.props.isInteractive
    );
  }
  ...
  renderWordColumn = (words, numResponses) => {
    ...
    for (let i = 0, len = words.length; i < len; i++) {
      if (i < numResponses) {
        arr.push(
          <li className={styles.listElt} key={i}>
            {words[i]}
          </li>
        );
      } else {
        ...
        skippedArr.push(
          <li
            className={[styles.listElt, styles.skippedWord].join(" ")}
            key={i}
          >
            {words[i]}
          </li>
    ...
    return (
      <div className={styles.fullColumn}>
        ...
        <ol className={[styles.unstyledList, styles.numberedList].join(" ")}>
          {" "}
          {arr}{" "}
        </ol>
      </div>
      ...
      arr.push(
        <li
          className={[
            ...
            words[i] === responses[i]
              ? styles.goodMetric
              : [styles.fairMetric, styles.wrong].join(" ")
          ].join(" ")}
          key={i}
        >
          {responses[i]}
  ...
  getSymbol = (data, sectionNum, wordIdx) => {
    if (data === true) {
      return (
        <i
          onClick={() => this.toggleSpellingError(sectionNum, wordIdx)}
          className={[
            "fa fa-check",
            styles.goodMetric,
            this.isStatusGradable(wordIdx) ? styles.clickable : ""
          ].join(" ")}
        />
      );
    }
  ...
  renderPhoneticColumn = (statusArr, title, sectionNum, numResponses) => {
    ...
    for (let i = 0, len = statusArr.length; i < len; i++) {
      if (i < numResponses) {
        arr.push(
          <li className={styles.listElt} key={i}>
            {this.getSymbol(statusArr[i], sectionNum, i)}
          </li>
        );
      }
    }
  ...
  renderSections = () => {
    ...
    for (let i = 1, len = this.state.spellingObj.numSections; i <= len; i++) {
      arr.push(
        this.renderPhoneticColumn(
          this.state.spellingObj.sections[String(i)].statusArr,
          this.state.spellingObj.sections[i].title,
          i,
          this.state.spellingObj.responses.length
        )
      );
    }
  ...
  render() {
    const popoverTop = (
      <Popover id="popover-positioned-top">
        <strong>Sorry!</strong> Rescoring developmental spelling is disabled for
        this demo.
      </Popover>
    );
    ...
    return (
      <div>
        <div className={styles.colsWrapper}>
          {this.renderWordColumn(
            this.state.spellingObj.words,
            this.state.spellingObj.responses.length
          )}

          {this.renderResponsesColumn(
            this.state.spellingObj.words,
            this.state.spellingObj.responses
          )}

          {this.renderSections()}
        </div>

        {!this.state.showSkippedWords &&
          this.state.spellingObj.responses.length !==
            this.state.spellingObj.words.length && (
            <span
              className={styles.toggleText}
              onClick={this.toggleShowSkippedWords}
            >
              {" "}
              See skipped words{" "}
              <i
        ...
        {this.state.showSkippedWords &&
          this.state.spellingObj.responses.length !==
            ...
            >
              Hide skipped words{" "}
              <i
                className={"fa fa-caret-up " + styles.caret}
                aria-hidden="true"
              />
            </span>
          )}
        ...
        {!this.props.isInteractive && (
          ...
          >
            <span
              onClick={this.onShowRescoreModal}
              style={{ marginLeft: 0 }}
              className={styles.spellingRescore}
            >
