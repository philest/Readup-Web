...
export default class CompQuestion extends React.Component {
  ...
  renderFullQuestion = () => {
    ...
    return (
      <div
        key={this.props.questionNum + "fullQ"}
        className={[
          styles.questionBlock,
          "faa-parent animated-hover faa-slow"
        ].join(" ")}
      >
        <h4
          className={[
            styles.questionText,
            this.props.isGraded ? "" : styles.fadedComp
          ].join(" ")}
        >
          {qLabel + this.getFullTitle()}
  ...
  renderGradedPartOfQuestion = questionNum => {
    ...
    if (this.state.savedNewScore) {
      // Use their new scoring instead
    ...
    return (
      <div className={"faa-parent animated-hover faa-slow"}>
        {this.props.studentResponse && (
          <p className={styles.studentResponse}>
            "{this.props.studentResponse}"
          </p>
        )}
        ...
        {!this.props.showCompAudioPlayback && (
          <Button
            onClick={() => this.props.onCompPlayRecordingClicked(questionNum)}
            className={[
              "fa faa-horizontal faa-slow",
        ...
        <p className={colorClass}>
          <span className={styles.correct}>{scoreLabel}:</span> {graderComment}
        </p>
        <span className={styles.rescoringLabels}>
          ...
          <span onClick={this.onShowRescoreModal} className={styles.rescore}>
            Give a different score
          </span>
        </span>
      </div>
    );
    ...
    return (
      <div
        className={[
          styles.questionBlock,
          "faa-parent animated-hover faa-slow",
          styles.rescoringQuestionBlock
        ].join(" ")}
      >
        <h4
          className={[
            styles.questionText,
            styles.rescoringQuestionText,
            this.props.isGraded ? "" : styles.fadedComp
          ].join(" ")}
        >
          {this.getFullTitle()}
  ...
  renderScoringButtonsComp = () => {
    ...
    for (let i = 0; i <= pointsPossible; i++) {
      buttonArr.push(
        <Button
          key={i}
          active={this.state.compScore === i}
          href="#"
          onClick={() => this.onCompScoreClicked(i, qNum - 1)}
        >
          <strong>{i}</strong> {i === 0 ? " points" : " points"}
        </Button>
      );
    }
  ...
  onSaveNewScoreClicked = () => {
    if (this.props.assessmentID) {
      ...
      assessment.then(assessment => {
        ...
        graderCommentsHolder[
          this.props.questionNum - 1
        ] = this.state.graderComment;
  ...
  render() {
    return (
      <div key={this.props.questionNum + "full"}>
        {this.renderFullQuestion()}

        <style type="text/css">{".modal-backdrop.in { opacity: 0.6; } "}</style>
        <Modal
          key={this.props.questionNum + "modal"}
          dialogClassName={styles.modalLg}
          show={this.state.showRescoreModal}
          onHide={this.onHideRescoreModal}
        >
          <Modal.Header
            bsClass={[styles.playbookModalHeader, "modal-header"].join(" ")}
            closeButton
          >
            <Modal.Title>
            ...
            <div className={styles.rescoringContainer}>
              <div className={styles.tophalfRescoring}>
                ...
                {this.props.studentResponse && (
                  <p
                    className={[
                      styles.studentResponse,
                      styles.rescoringStudentResponse
                    ].join(" ")}
                  >
                    "{this.props.studentResponse}"
                  </p>
              ...
              <FormGroup controlId="graderComments">
                ...
                <FormControl
                  ...
                  className={styles.rescoringGraderCommentForm}
                  value={this.state.graderComment}
                  onChange={event => this.handleGraderCommentChange(event)}
