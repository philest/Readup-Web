...
RSpec.describe "homepage", type: :view do
  ...
  it 'presents the login modal' do
    visit '/'
    click_button 'Log in'
  ...
  it 'requires valid login information' do
    visit '/'
    click_button 'Log in'
  ...
  it 'requires valid username on signup' do
    visit '/'
    find('#main-signup-button').click
    click_button 'Continue'
  ...
  it 'advances to signup enter name and password given valid username' do
    visit '/'
    find('#main-signup-button').click
    # find(:xpath, '//input[@name="usernameDisplay"]').set 'capybara-test-new-user@readupapp.com'
